...
export function Navbar({ onMenuClick }: { onMenuClick?: () => void }) {
  ...
  return (
    <header className="h-16 bg-[var(--bg-surface)] border-b border-[var(--border-subtle)] flex items-center px-4 gap-4 z-30 flex-shrink-0">
      {/* Mobile menu button */}
      <button
      ...
      {/* Page title */}
      <h1 className="text-[var(--text-primary)] font-semibold text-lg hidden sm:block">{title}</h1>
      ...
      <div className="flex-1 max-w-xs hidden lg:flex items-center ml-4">
        <div className="relative w-full">
          ...
          <input
            ...
            className="w-full bg-[var(--bg-surface-2)] border border-[var(--border-subtle)] rounded-xl pl-9 pr-4 py-2 text-sm text-[var(--text-primary)] placeholder:text-[var(--text-secondary)] focus:outline-none focus:border-[var(--accent-gold)] transition-colors"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative" ref={userRef}>
          ...
          >
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[var(--accent-gold)] to-amber-600 flex items-center justify-center">
              <span className="text-xs font-bold text-[#0A0A0F]">{initials}</span>
            </div>
